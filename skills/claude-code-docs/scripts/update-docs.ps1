...
# 把所有分类点开后还需要全部展开后再运行（特别是Getting started好几个需要展开的）
# copy(Array.from(document.querySelectorAll("#navigation-items a")).filter(i=>i.href.startsWith("https://code.claude.com/docs/en/")).map(i=>i.href).join("\n"))
# 可以参考：https://code.claude.com/docs/llms.txt
...
for ($i = 0; $i -lt $DocLinks.Count; $i++) {
    ...
    $job = Start-ThreadJob -ScriptBlock {
        ...
        try {
            ...
            $response.Content -split "`r?`n" | Select-Object -Skip 3 | Set-Content -Path $path -Encoding UTF8
