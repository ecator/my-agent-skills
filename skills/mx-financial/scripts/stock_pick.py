...
def _build_column_map(columns: list[dict[str, Any]]) -> dict[str, str]:
    ...
    for col in columns or []:
        ...
        cn_name = (
            col.get("displayName", "") or col.get("title", "") or col.get("label", "")
        )
...
async def query_MX_StockPick(query: str, selectType: str) -> dict[str, Any]:
    """
    ...
    try:
        ...
        result["error"] = f"MCP call failed: {e!s}"
        return result

    if not raw or not isinstance(raw, dict):
        result["error"] = "MCP returned empty or non-JSON object"
    ...
    inner_result = all_results.get("result") or {}

    dataList = inner_result.get("dataList")
    ...
    if not rows:
        result["error"] = "No valid datalist returned and partialResults cannot be parsed or is empty"
...
def run_cli() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Stock, sector, and fund screening via natural language query"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Natural language query, e.g., 'Stocks with a price greater than 1000'",
    )
    ...
    parser.add_argument(
        ...
        help="Specify target type for screening",
    )
    args = parser.parse_args()

    async def _main() -> None:
        r = await query_MX_StockPick(args.query, selectType=args.selectType)
