...
<Warning>
  The **New seat tier** button only appears if your tenant has allowed organizations to manage their own seat tiers. This permission is controlled by the **Organizations manage their own seat tiers** setting on the tenant's Config page. If you don't see the button, ask a tenant administrator.
</Warning>
...
You can only delete a tier that nothing references. If any users are still assigned to it, any API keys are bound to it, or any group mapping on the [Group mappings](/government/org-admin/provisioning) page points at it, the delete is refused with a message telling you so. Reassign or remove those references first, then delete the tier.
