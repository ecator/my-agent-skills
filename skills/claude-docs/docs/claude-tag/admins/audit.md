...
The [See it work](/claude-tag/admins/test-it) page uses this check to validate a new connection.
...
Routines run with the channel's credentials, so the channel listing is also the permission picture. See [proactivity](/claude-tag/users/proactivity#manage-standing-work).

## Related resources

* [How agent identity works](/claude-tag/concepts/agent-identity): how attribution differs in channels and DMs
* [Restrict where Claude Tag operates](/claude-tag/admins/restrict-access): the controls when an audit turns something up
* [Security and data handling](/claude-tag/concepts/security-and-data): the model behind the trails
