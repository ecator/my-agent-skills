...
Check that the channel has the connections below. Ask `@Claude what can you access from this channel?` to check; an admin can [add a connection](/claude-tag/admins/add-connections) the channel is missing. Each recipe below names the connection it uses.
...
"Remember for this channel" saves the role to [channel memory](/claude-tag/users/memory), so it applies to everyone's threads, not just yours. [Triage requests](/claude-tag/users/use-cases/triage-requests) adds a weekly rollup that also sweeps posts that never tagged Claude.
...
Name the format, the length, and the source the draft should match. The same prompt shape produces a decision doc or a status memo; [Turn threads into docs and tickets](/claude-tag/users/use-cases/create-artifacts) lists the variants.
...
The reply lists the stalled records and the stage where each stopped. The [HubSpot connection](/claude-tag/admins/connections/hubspot) is created with read scopes, so prompts in this channel pull records and don't change them. For account questions, pre-call briefs, and a pipeline digest, see [Pull deal and account state](/claude-tag/users/use-cases/pull-deal-state).
...
Naming the timezone matters, since schedules default to UTC. To list or cancel scheduled work later, see [Manage standing work](/claude-tag/users/proactivity#manage-standing-work).
