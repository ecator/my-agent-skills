...
The pull request appears under the Claude GitHub App and links back to the Slack thread it came from. See [how agent identity works](/claude-tag/concepts/agent-identity#agent-access).
...
With the if/can't branch, every report gets a reply, either a draft or a list of what was tried. To list or cancel scheduled work later, see [Manage standing work](/claude-tag/users/proactivity#manage-standing-work).
...
For repository conventions that should hold across every session that touches the code, like where files go or what a pull request must include, see [Make repo conventions stick](/claude-tag/users/good-habits#teach-claude-something-that-sticks).
