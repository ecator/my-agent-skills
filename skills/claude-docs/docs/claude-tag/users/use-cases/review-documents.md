...
Claude can reach only what the connected account can see in that tool. If a document is missing from a review, ask an admin to [share it with the connected account](/claude-tag/admins/add-connections#limit-access-to-specific-resources).
...
Including the timezone matters, since schedules default to UTC. To list or cancel scheduled work later, see [Manage standing work](/claude-tag/users/proactivity#manage-standing-work).

Keep the checklist as a document Claude can read, in the connected tool or linked in the channel, rather than re-describing its contents in [channel memory](/claude-tag/users/memory).

## Related resources

<CardGroup cols={2}>
  <Card title="Find answers in your docs" href="/claude-tag/users/use-cases/find-answers" horizontal arrow>
