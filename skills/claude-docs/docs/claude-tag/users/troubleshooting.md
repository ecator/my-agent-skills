...
You set up and authenticate connectors on claude.ai under **Settings > Connectors**; Slack has no connector settings of its own.

**How to resolve**

For a channel, ask your admin to [add a connection](/claude-tag/admins/add-connections) for the service.
...
3. For a custom connector on a Team or Enterprise plan, an Owner adds it to the organization before you can connect it; see [third party connectors with remote MCP](/connectors/custom/remote-mcp).
4. After connecting, start a new thread; a session loads its connectors when it starts.
