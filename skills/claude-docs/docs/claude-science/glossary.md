...
**[Annotation](/claude-science/annotations)**: a comment you pin to part of an artifact by selecting it; Claude reads pending annotations as instructions on its next turn.

**[Artifact](/claude-science/artifacts)**: any file Claude makes and saves for you, listed in its session's Files view.
...
**[Environment](/claude-science/tools-and-environments)**: a named set of installed packages (conda) that a kernel runs in, reused across sessions.
...
**[Reviewer](/claude-science/the-reviewer)**: the independent agent that re-examines Claude's claims and artifacts at checkpoints, recording each issue it raises as a finding on the artifact's Review tab; on some plans it runs in the background automatically.
