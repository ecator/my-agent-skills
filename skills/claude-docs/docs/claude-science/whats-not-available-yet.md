...
Claude Science is in beta, and some admin controls you use with other Claude products don't govern it yet. The setup wizard lists some of these when you enable the product; this page has the full detail. See [Admin controls](/claude-science/admin-controls) for the complete per-setting table.

## Audit and compliance

Audit log: no Claude Science events are written to the organization audit log yet. This is on the roadmap.\
Compliance API: admins can't export or delete Claude Science data through the Compliance API. Because conversations live on members' computers, there's no Anthropic-side data for the API to reach; see [Manage on devices](/claude-science/manage-on-devices) for endpoint-level options.\
