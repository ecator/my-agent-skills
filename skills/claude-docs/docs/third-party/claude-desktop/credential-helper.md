...
Configure the helper with the `inferenceCredentialHelper` key; see the [Configuration reference](/third-party/claude-desktop/configuration#inferencecredentialhelper) for the full list of helper-related keys.
...
* **A JSON object**, when per-request headers are needed:
  ...
  Headers from the JSON object are merged over [`inferenceCustomHeaders`](/third-party/claude-desktop/configuration#inferencecustomheaders); the helper's value wins on a conflict.
