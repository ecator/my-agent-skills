...
| You already operate an LLM proxy                                                                                      | [Gateway provider](/third-party/claude-desktop/gateway) instead of Google Cloud's Agent Platform                       | None                                 | At your gateway                    | The proxy holds the Google Cloud credentials; the app authenticates only to the proxy.                                                   |
...
Under **Models**, add at least one **Model list** entry using the publisher model ID, for example `claude-sonnet-4-5@20250929`.
...
You must also set `inferenceModels` to a list of publisher model IDs, for example `claude-sonnet-4-5@20250929`. See the [Configuration reference](/third-party/claude-desktop/configuration#inferencemodels).
