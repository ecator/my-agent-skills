...
# 文档页面控制台执行
# copy(Array.from(document.querySelectorAll(".sidebar-content a")).filter(i=>i.href.startsWith("https://opencode.ai/docs/")).map(i=>i.href.replace(/zh-cn\/([^\/]+)\/?$/,'$1.md')).join("\n"))
# 第一个URL需要特殊处理下，变成/index.md
