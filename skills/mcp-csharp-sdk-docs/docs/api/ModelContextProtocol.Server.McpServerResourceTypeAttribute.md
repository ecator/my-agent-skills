...
Indicates that a type contains members that should be exposed as [McpServerResource](ModelContextProtocol.Server.McpServerResource.html)s.
...
discovered and registered as [McpServerResource](ModelContextProtocol.Server.McpServerResource.html)s. When combined with discovery methods like
...
resources must be marked with the [McpServerResourceAttribute](ModelContextProtocol.Server.McpServerResourceAttribute.html).
