...
Specifies that a type contains methods that should be exposed as [McpServerPrompt](ModelContextProtocol.Server.McpServerPrompt.html)s.
...
discovered and registered as [McpServerPrompt](ModelContextProtocol.Server.McpServerPrompt.html) prompts. When combined with discovery methods like
...
prompts must be marked with the [McpServerPromptAttribute](ModelContextProtocol.Server.McpServerPromptAttribute.html).
