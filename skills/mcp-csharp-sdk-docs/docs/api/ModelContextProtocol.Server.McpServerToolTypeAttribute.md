...
Indicates that a type contains methods that should be exposed as [McpServerTool](ModelContextProtocol.Server.McpServerTool.html)s.
...
discovered and registered as [McpServerTool](ModelContextProtocol.Server.McpServerTool.html)s. When combined with discovery methods like
...
tools must be marked with the [McpServerToolAttribute](ModelContextProtocol.Server.McpServerToolAttribute.html).
